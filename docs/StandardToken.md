...
### <span id="StandardToken-transfer-address-uint256-"></span> `transfer(address _to, uint256 _value) → bool` (external)

Transfers sender's tokens to a given address, added due to backwards compatibility reasons with ERC20


### <span id="StandardToken-transferFrom-address-address-uint256-"></span> `transferFrom(address _from, address _to, uint256 _value) → bool` (external)

Allows allowed third party to transfer tokens from one address to another. Returns success.


### <span id="StandardToken-balanceOf-address-"></span> `balanceOf(address _owner) → uint256` (external)

Returns number of tokens owned by given address.


### <span id="StandardToken-approve-address-uint256-"></span> `approve(address _spender, uint256 _value) → bool` (external)

Sets approved amount of tokens for spender. Returns success.


### <span id="StandardToken-allowance-address-address-"></span> `allowance(address _owner, address _spender) → uint256` (external)

Returns number of allowed tokens for given address.


### <span id="StandardToken-transfer-address-uint256-bytes-"></span> `transfer(address _to, uint256 _value, bytes _data) → bool` (public)
